...
class lap_pyramid():
    def __init__(self, k, image_size, filt=np.reshape([[0.0025, 0.0125, 0.0200, 0.0125, 0.0025],
    	...
    	self.k = k
    	self.filt = torch.Tensor(filt).cuda()
    ...
    def DN_filters(self):
        ...
        dn_filts.append(torch.Tensor(np.reshape([[0, 0.1011, 0],
                                    [0.1493, 0, 0.1460],
                                    [0, 0.1015, 0.]]*3,
                                   (3,	1, 3, 3)).astype(np.float32)).cuda())
    ...
    def compare(self, x1, x2):
        y1 = self.pyramid(x1)
        y2 = self.pyramid(x2)
        total = [torch.sqrt(torch.sum((z1 - z2) ** 2)) for z1, z2 in zip(y1, y2)]
        return torch.sum(torch.stack(total)) / self.k
